{{-- Variables: $content['title'], $settings['show_phone'], $settings['show_address'],
     $settings['show_hours'], $profile (SalonProfile, may be null), $block --}}
    ...
    <div class="sf-inner" style="max-width:720px">
        ...
        @if($profile)
            @if($settings['show_address'] ?? true)
                @if($profile->address || $profile->phone || $profile->whatsapp_number || $profile->instagram_url || $profile->facebook_url)
            ...
            @if($settings['show_hours'] ?? true)
                ...
                @endphp
                <ul class="sf-hours-list" style="margin-top:1.5rem;list-style:none;padding:0">
            ...
            <div class="sf-map-wrap" style="margin-top:2rem" x-data="{ loaded: false }">
                ...
                <template x-if="loaded">
                    <iframe src="{{ $profile->google_maps_embed }}"
                        ...
                        style="width:100%;height:300px;border:0;border-radius:var(--sf-radius,8px)"></iframe>
                </template>
